...
#		self.torSpeedDif = sp.Register ()
	...
	def sweep (self):
		# Torso
		self.torAngDif.set (self.torAng - self.torAngSet)										# Changed
...
#		self.torVolt.set (sp.limit (self.torVoltFac * self.torSpeedDif, self.torVoltMax))
		self.torBrakeDist.set (self.torBrakeFac * self.torSpeed * self.torSpeed)				# New
		self.torVolt.set (																		# New	111 B 222 T 333 B 444	B = breaking point, T = target
			# Value if condition
			self.torVoltMax,
			
			# Condition
			self.torAngDif < -self.torBrakeDist or												# Area 1: Force to right always
			(-self.torBrakeDist < self.torAngDif < self.torBrakeDist and self.torSpeed < 0),	# Area 2 & 3: Force to right if speed points to left
			
			# Value if not condition
			-self.torVoltMax
		)
		...
		self.foreRound = sp.abs (self.forAngDif) < self.forMarg
		...
		self.wrieRound = sp.abs (self.wriAngDif) < self.wriMarg
